...
import (
	...
	"github.com/Harman6282/students-api/internal/storage"
...
func UpdateById(storage storage.Storage) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		student, err = storage.UpdateStudent(student.Name, student.Email, student.Age, intId)
		if err != nil {
			response.WriteJson(w, http.StatusInternalServerError, response.GeneralError(err))
			return
		}
...
func DeleteById(storage storage.Storage) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		slog.Info("starting delete a student")
		...
		response.WriteJson(w, http.StatusOK, fmt.Sprintf("Student deleted with id %v", intId))
